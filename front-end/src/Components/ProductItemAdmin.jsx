...
const ProductCard = ({ img, brand, setreload, imgOnHover, title, salePrice, type, size, color, images, category, id, regularPrice }) => {
    ...
    const handleDelete = () => {
        ...
        axios
            .delete(`${process.env.REACT_APP_BACKEND_API}/${type}/delete/${id}`, {
                headers: {
    ...
    return (
        <>
            <BasicUsage brand={brand} title={title} price={salePrice} type={type} color={color} images={images} size={size} category={category} id={id} isOpen={isOpen} setreload={setreload} onOpen={onOpen} onClose={onClose} />
            <Card direction={{ base: "column", sm: "row" }} overflow="hidden" variant="outline" position="relative" onMouseEnter={() => sethover(true)} onMouseLeave={() => sethover(false)}>
                <Box bg="red" color={"white"} position={"absolute"} p="4px 8px" fontSize={"9px"} fontWeight="bold" borderRadius={"50px"} top="10px" left="10px" zIndex="10">
                    - 50%
                </Box>
                <Box position="relative" w="" class="image-wrapper profile-pic">
                    <Image
                        ...
                        alt="normal"
                    />
                    <Image objectFit={"contain"} position={"absolute"} top="0px" transition="opacity 0.5s ease-in-out" src={imgOnHover} className="image" opacity={hover ? "1" : "0"} alt="normal" />
                </Box>
                ...
                <Stack w="100%">
                    <CardBody>
                        ...
                        <Flex pt="10px" fontSize={"15px"}>
                            ...
                            {regularPrice ? <Text ml="5px"> | Original Price Rs {regularPrice}.00</Text> : <></>}
                        </Flex>

                        <Text textAlign="left" pt="10px" fontSize={"14px"} fontWeight="bold">
                            Product ID : {id}
                        </Text>
                        <Divider />
                        <Flex mt="20px">
...
function BasicUsage({ brand, title, price, type, color, images, size, category, id, isOpen, onOpen, onClose, setreload }) {
    return (
        <>
            <Modal isOpen={isOpen} onClose={onClose}>
                ...
                <ModalContent>
                    ...
                    <ModalCloseButton />
                    <ModalBody>
                        <AddProducts brand1={brand} title1={title} price1={price} type1={type} setreload={setreload} color1={color} images={images} category1={category} size1={size} id={id} />
                    </ModalBody>
                </ModalContent>
            </Modal>
        </>
...
const AddProducts = ({ brand1, title1, price1, type1, color1, setreload, size1, images, category1, id }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setreload((prev) => !prev);
        try {
            await axios.patch(`${process.env.REACT_APP_BACKEND_API}/${type}/update/${id}`, obj, {
                headers: {
