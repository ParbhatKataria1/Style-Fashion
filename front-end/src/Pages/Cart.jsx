            ...
            <Box w={"25%"}>
                ...
                <Box border={"1px solid lightgray"} borderRadius="5px" p={"15px"} boxShadow={"rgba(60, 64, 67, 0.3) 0px 1px 2px 0px, rgba(60, 64, 67, 0.15) 0px 2px 6px 2px"}>
                    <Flex direction="column">
                        ...
                        <PaymentModal subtotal={subtotal} tax={tax} totalPrice={totalPrice + tax} />
                    </Flex>
                </Box>
            </Box>
        </>
    ...
    const updateQty = async (id, qty) => {
        ...
        try {
            let data = await axios.patch(
                `${process.env.REACT_APP_BACKEND_API}/cart/update/${id}`,
                { qty: +qty },
                {
                    headers: {
                        Authorization: process.env.REACT_APP_TOKEN,
...
const CartItem = ({ cartItem, updateQty, handleDelete }) => {
    ...
    return (
        <Box mb={"10px"} border="1px solid lightgray" borderRadius={"5px"}>
            <Flex justifyContent={"flex-start"} borderTop={"1px solid lightgray"} borderBottom={"1px solid lightgray"} p="10px">
                <Box w="30%" mr="70px" borderRadius={"5px"} border={"1px solid black"} bg="black" boxShadow={"rgba(0, 0, 0, 0.35) 0px 5px 15px;"}>
                    <Flex w="100%" justifyContent={"center"} alignItems="center" h="200px">
                        <Image h={"100%"} objectFit="cover" src={cartItem.images[0]} alt={cartItem.title}></Image>
                    </Flex>
                </Box>
                ...
                <Box cursor={"pointer"} onClick={() => handleDelete(cartItem._id)} w="5%">
                    <svg w="10px" h="10px" clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2" xmlns="http://www.w3.org/2000/svg">
