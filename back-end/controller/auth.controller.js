...
const register = async(req, res)=>{
    ...
    try {
        ...
        if(data){
            ...
            let {password:hash,_id} = await AuthModel.findOne({first_name, email,last_name});
            ...
            bcrypt.hash(first_name+last_name, 5, async(err, hash)=> {
                ...
                if(hash){
                    let user = new AuthModel({first_name, email, last_name, password:hash});
                    await user.save();
                    const {_id} = await AuthModel.findOne({first_name, email,last_name});
                    if(_id)res.status(200).send({"msg":"user is logged in2 ","token":jwt.sign({ userId: _id }, 'masai')})
                    else res.status(400).send({"msg":"login details are wrong or not able to compare the hash2"});
                }
