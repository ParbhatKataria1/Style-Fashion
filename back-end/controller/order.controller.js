...
const getPosts = async(req, res)=>{
    ...
    //    }
       let data;
       if(page){
           data = await OrderModel.find(obj).skip((page-1)*limit).limit(limit);
       }
       else {
...
const updateData = async(req, res)=>{
    try {
        ...
        let body = req.body;
        let {userId}= req.body;
        let data = await OrderModel.findOneAndUpdate({_id, userId}, body, {new:true});
        console.log(data);
...
const deleteData = async(req, res)=>{
    try {
        let {_id} = req.params;
        let {userId}= req.body;
        let data = await OrderModel.findOneAndDelete({_id, userId}, null, {new:true});
        console.log(data);
